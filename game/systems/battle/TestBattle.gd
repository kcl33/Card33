...
func _ready():
	...
	# 初始化游戏控制器
	game_controller = preload("res://path/to/GameController.gd").new()
	battle_manager = preload("res://path/to/BattleManager.gd").new()
	card_effect_system = preload("res://path/to/CardEffect.gd").new()
	forge_cards = preload("res://path/to/ForgeCardExamples.gd").new()
	...
	# 初始化鼠标控制器
	mouse_controller = preload("res://path/to/MouseController.gd").new(game_controller)
...
func initialize_ui_areas():
	# 手牌区域 - 玩家1（自己）
	hand_area_player1 = HandArea.new(game_controller.player1, true)  # true表示是玩家手牌
	add_child(hand_area_player1)
	
	# 手牌区域 - 玩家2（对手）
	hand_area_player2 = HandArea.new(game_controller.player2, false)  # false表示是对手手牌
	add_child(hand_area_player2)
	
	# 场上区域
	field_area_player1 = FieldArea.new(game_controller.player1)
	add_child(field_area_player1)
	
	field_area_player2 = FieldArea.new(game_controller.player2)
	add_child(field_area_player2)
	
	# 场地可视化区域
	field_visual_player1 = FieldVisual.new(game_controller.player1)
	add_child(field_visual_player1)
	
	field_visual_player2 = FieldVisual.new(game_controller.player2)
	add_child(field_visual_player2)

# 初始化抽卡动画
func initialize_draw_animations():
	draw_animation_player1 = DrawCardAnimation.new()
	...
	draw_animation_player2 = DrawCardAnimation.new()
