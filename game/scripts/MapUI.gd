...
func _generate_map():
	...
	# 生成新地图
	var nodes = map_generator.generate_map()
	
	# 将节点添加到容器中
	for node in nodes.values():
		map_container.add_child(node)
		# 连接节点点击信号（Godot 4）
		node.gui_input.connect(_on_node_clicked.bind(node))
...
# 节点点击处理
func _on_node_clicked(event: InputEvent, node):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		select_next_node(node.node_id)
